...
$(document).ready(function(){
    ...
    $.get('/house/index_get/', function(data){
        var area_str = template('area_script_id', {areas: data.area_list});
        $('.filter-area').append(area_str);
    })

    var search_url = location.search
    $.get('/house/search_get/' + search_url, function(data){
        if(data.code == '200'){
            var house_str = template('house_list_id', {houses: data.houses_dict});
